...
const SceneSidebar = ({ activeScene, onSceneSelect }: SceneSidebarProps) => {
  return (
            ...
            >
              {isActive && (
                <motion.div
                  ...
                  style={{ boxShadow: "0 0 8px rgba(74,157,164,0.5)" }}
                  transition={{ type: "spring", stiffness: 500, damping: 30 }}
                />
              )}
              <span className={`font-mono text-[10px] transition-colors ${isActive ? "text-primary" : "text-muted-foreground group-hover:text-primary"}`}>
                0{scene.id}
              </span>
              <span className={`flex-1 text-sm font-medium tracking-tight transition-colors ${isActive ? "text-foreground" : "text-foreground/60 group-hover:text-foreground"}`}>
                {scene.name}
              </span>
              <span className="font-mono text-[10px] text-muted-foreground">{scene.duration}</span>
            </motion.button>
          );
        })}
      </nav>
